...
COMMENT_HEADER = '### ✅ Definition of Done checker 📖'
EMPTY_CHECKMARK = '- [ ]'
...
def has_unsatisfied_criteria(bot_comment):
    return EMPTY_CHECKMARK in bot_comment

def main():
    parser = argparse.ArgumentParser(
        description='Definition of Done checker')
    ...
    comments_url = '%s/repos/%s/issues/%s/comments' % (
        github_api_url, repo, pull_request_id)
    
    get_result = requests.get(comments_url)
    if get_result.status_code != 200:
        print('Getting pull request comments to GitHub failed with code: ' +
              str(get_result.status_code))
        print(get_result.text)

    bot_comment = get_bot_comment(get_result.json())
    if bot_comment:
        if has_unsatisfied_criteria(bot_comment):
            print('The pull request has unsatisfied criteria, please make sure to check them off.')
            return 1
        else:
            print('All DoD criteria are satisfied. 🎉')
            return 0
    else:
        print('The pull request has unsatisfied criteria, please make sure to check them off in the comment that was just posted.')
        message = dod_criteria_to_message(config['dod'])
        post_result = requests.post(comments_url, json={'body': message}, headers={
                                    'Authorization': 'token %s' % github_token})
        if post_result.status_code != 201:
            print('Posting comment to GitHub failed with code: ' +
